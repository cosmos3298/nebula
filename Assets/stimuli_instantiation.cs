...
public class stimuli_instantiation : MonoBehaviour {
	...
	public static void exp_initiator () {
		...
		// To set the zero point for presenting an array of stimuli 
		int[,] zeroing_point = new int[,] { {-10, 7} };

		// To randomize stimuli. This section(from here to the line of stars) could be replaced with desired pattern of stimuli presentation,
		// such as setting an array to indicate the sequence of presentation
		...
		for (int x = 0; x < number_of_x; x ++) {
			for (int y = 0; y < number_of_y; y ++) {
				...
				// To instantiate the bait according to the previously assigned array of baits
